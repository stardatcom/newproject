...
const ControladorOrdenDetrabajo = {
    ...
    try {
      ...
      if (ordenDeTrabajoCreado._id) {
        res.json({
          ...
          datos: ordenDeTrabajoCreado._id,
        });
      }
    } catch (error) {
      res.json({
  ...
  leerOrdenDeTrabajo: async (req, res) => {
    try {
      const ordenDeTrabajoEncontrado = await ModeloOrdenDetrabajo.findById(
        req.params.id
      );
  ...
  eliminarOrdenDeTrabajo: async (req, res) => {
    try {
      const ordenDeTrabajoEliminada =
        await ModeloOrdenDetrabajo.findByIdAndDelete(req.params.id);
